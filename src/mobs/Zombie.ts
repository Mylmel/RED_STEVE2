...
export class Zombie extends Mob {
  ...
  // Новые свойства для большого зомби
  private isBig: boolean = false;
  private bigScale: number = 3.0;
  private bigDamage: number = 5; // Урон большого зомби
  private bigHealth: number = 50; // Здоровье большого зомби
  private bigWalkSpeed: number = 1.2; // Скорость большого зомби (немного медленнее)
  ...
  constructor(
    ...
    y: number,
    z: number,
    isBig: boolean = false // Новый параметр для создания большого зомби
  ) {
    super(world, scene, x, y, z);
    
    // Устанавливаем тип зомби
    this.isBig = isBig;
    
    // Если большой зомби, увеличиваем здоровье
    if (this.isBig) {
      this.health = this.bigHealth;
    }

    const texture = world.noiseTexture;
    
    // Разные цвета для обычного и большого зомби
    let skinColor, shirtColor, pantsColor;
    
    if (this.isBig) {
      skinColor = [0.8, 0.1, 0.1]; // Красный для большого зомби
      shirtColor = [0.5, 0.1, 0.1]; // Темно-красный
      pantsColor = [0.3, 0.1, 0.1]; // Очень темный красный
    } else {
      skinColor = [0.2, 0.6, 0.2]; // Зеленый для обычного зомби
      shirtColor = [0.2, 0.2, 0.8]; // Синий
      pantsColor = [0.2, 0.2, 0.6]; // Темно-синий
    }
    ...
    // Если это большой зомби, применяем масштаб
    if (this.isBig) {
      this.mesh.scale.set(this.bigScale, this.bigScale, this.bigScale);
      
      // Для большого зомби нужно скорректировать позицию, чтобы он стоял на земле
      this.mesh.position.y += 1.0; // Поднимаем немного, т.к. он выше
    }
  ...
  protected updateAI(
    ...
    if (isDay && !this.isDead) {
      ...
      const y = Math.floor(this.mesh.position.y + (this.isBig ? 3.0 : 1.8)); // Check from head up
    ...
    if (isMoving) {
      const speed = this.isBig ? 7 : 10; // Большой зомби медленнее машет конечностями
    ...
    if (this.state === MobState.SEEK_SHELTER && this.targetShelter) {
      ...
      if (dToShelter > 0.5) {
        ...
        const currentSpeed = this.isBig ? this.bigWalkSpeed : this.walkSpeed;
    ...
    if (this.state === MobState.CHASE) {
      ...
      const checkDist = this.isBig ? 1.5 : 1.0; // Большой зомби проверяет дальше
      ...
      const isBlocked = (
        ...
        // Для большого зомби проверяем больше блоков по высоте
        if (this.isBig) {
          for (let i = 0; i < 3; i++) {
            if (this.world.hasBlock(tx, ty + i, tz)) {
              return true;
            }
          }
          return false;
      ...
      const attackRange = this.isBig ? 3.0 : 2.0; // Большой зомби имеет большую дальность атаки
      const currentSpeed = this.isBig ? this.bigWalkSpeed : this.walkSpeed;
      ...
      if (dist < attackRange * 0.7) {
        ...
        const pushSpeed = this.isBig ? 7.0 : 5.0; // Большой зомби толкает сильнее
        this.velocity.x += pushDir.x * pushSpeed;
        this.velocity.z += pushDir.z * pushSpeed;
      }

      if (dist < attackRange * 1.1) {
        // Line of Sight Check
        const eyePos = this.mesh.position.clone();
        eyePos.y += this.isBig ? 3.0 : 1.6; // Глаза большого зомби выше
        ...
        if (!blocked) {
          ...
          const attackCooldown = this.isBig ? 2000 : 1500; // Большой зомби атакует реже
          
          if (now - this.lastAttackTime > attackCooldown) {
            this.state = MobState.ATTACK;
            const damage = this.isBig ? this.bigDamage : 2;
            ...
            // Звуковой эффект для большого зомби (можно добавить позже)
            if (this.isBig) {
              console.log("Большой зомби рычит!");
            }
          }
        }
      }
  ...
  private findNearbyShelter(): THREE.Vector3 | null {
    ...
    for (let x = -range; x <= range; x++) {
      for (let z = -range; z <= range; z++) {
        ...
        // Для большого зомби нужно больше места
        if (this.isBig) {
          if (
            this.world.hasBlock(cx, startY, cz) ||
            this.world.hasBlock(cx, startY + 1, cz) ||
            this.world.hasBlock(cx, startY + 2, cz)
          )
            continue;
        } else {
          if (
        ...
        // 3. Must have roof within reasonable height
        let hasRoof = false;
        const roofStart = this.isBig ? 3 : 2;
        for (let k = roofStart; k <= 10; k++) {
  ...
  protected onHorizontalCollision() {
    if (this.isOnGround) {
      ...
      const checkDist = this.isBig ? 1.2 : 0.8;
      ...
      // Для большого зомби проверяем больше блоков
      if (this.isBig) {
        if (
          !this.world.hasBlock(x, y + 1, z) &&
          !this.world.hasBlock(x, y + 2, z) &&
          !this.world.hasBlock(x, y + 3, z)
        ) {
          this.velocity.y = Math.sqrt(2 * 20.0 * 1.5);
  ...
  // Геттер для проверки типа зомби
  public getIsBig(): boolean {
    return this.isBig;
  }
  
  // Метод для получения урона в зависимости от типа зомби
  public getAttackDamage(): number {
    return this.isBig ? this.bigDamage : 2;
  }
}